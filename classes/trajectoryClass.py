...
class Trajectory:
    ...
    def simAnnealingAdd(self, var, score, newScore, T, addRail):
        ...
        """
        if SimulatedAnnealing.acceptance(score, newScore, helpers.Files.multiplicationAdd * T) == True:
            if self.minutesTrajectory() < helpers.Files.maxMinutes:
                if var == "begin":
    ...
    def simAnnealingChop(self, line, T):
        ...
        if len(self.Raillist) > 1:
            ...
            if SimulatedAnnealing.acceptance(score, choppedScore,  helpers.Files.multiplicationChop * T) == True:
                self.removeRailbyRailBeginning()
